    ...
    "relative w-full rounded-lg border border-gray-200 px-4 py-3 text-sm [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-gray-950 [&>svg~*]:pl-7 dark:border-gray-800 dark:[&>svg]:text-gray-50",
    {
        variants: {
            variant: {
                ...
                destructive:
                    "border-red-700/50 text-red-700 dark:border-red-700 [&>svg]:text-red-700 dark:border-red-900/50 dark:text-red-900 dark:dark:border-red-900 dark:[&>svg]:text-red-950",
            },
        },
        defaultVariants: {
            variant: "default",
...
>(({ className, ...props }, ref) => (
    <h5
        ref={ref}
        className={cn(
            "mt-1 mb-1 font-medium leading-none tracking-tight",
            className
        )}
        {...props}
    />
